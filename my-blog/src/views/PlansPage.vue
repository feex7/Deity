...
const form = reactive({ title: '', description: '', status: '规划中', priority: '中', tags: '' })

const openAdd = () => {
  editingId.value = null
  Object.assign(form, { title: '', description: '', status: '规划中', priority: '中', tags: '' })
  showForm.value = true
}
...
<template>
  <div class="page">
    <header class="page-header">
      <div>
        <h1>计划</h1>
        <p class="subtitle">未来想做的事情</p>
      </div>
      <button v-if="isAdmin()" class="btn-add" @click="openAdd">+ 添加计划</button>
    </header>
    ...
    <div v-else-if="items.length === 0" class="empty">暂无计划</div>

    <div v-else class="grid">
      <article v-for="item in items" :key="item.id" class="card">
        <div class="card-top">
          <span class="priority" :class="'p-' + item.priority">{{ item.priority === '高' ? '▲' : item.priority === '低' ? '▼' : '●' }} {{ item.priority }}</span>
    ...
    <Teleport to="body">
      <div v-if="showForm" class="overlay" @click.self="showForm = false">
        <div class="modal">
          <h3 class="modal-title">{{ editingId ? '编辑计划' : '添加计划' }}</h3>
          <input v-model="form.title" placeholder="计划名称" class="input" />
          <textarea v-model="form.description" placeholder="详细描述" class="input textarea" rows="3"></textarea>
          <div class="form-row">
            <select v-model="form.status" class="input half">
              <option v-for="s in statusOptions" :key="s" :value="s">{{ s }}</option>
            </select>
            <select v-model="form.priority" class="input half">
              <option v-for="p in priorityOptions" :key="p" :value="p">{{ p }}优先级</option>
            </select>
          </div>
...
.card { background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.06); border-radius: 14px; padding: 22px; transition: border-color .3s, transform .3s; }
.card:hover { border-color: rgba(0,255,128,0.15); transform: translateY(-2px); }
.card-top { display: flex; align-items: center; justify-content: space-between; margin-bottom: 12px; }
.priority { font-size: 11px; }
.p-高 { color: #ff6644; }
.p-中 { color: #ffaa22; }
.p-低 { color: rgba(255,255,255,0.3); }
.status-tag { font-size: 11px; padding: 3px 10px; border-radius: 10px; background: rgba(255,255,255,0.04); color: rgba(255,255,255,0.35); }
.status-tag.active { background: rgba(0,255,128,0.08); color: #00ff80; }
.status-tag.done { background: rgba(100,149,237,0.1); color: #6495ed; }
.status-tag.paused { background: rgba(255,255,255,0.04); color: rgba(255,255,255,0.2); }
.card-title { font-size: 15px; font-weight: 600; color: #fff; margin-bottom: 8px; }
.card-desc { font-size: 12px; color: rgba(255,255,255,0.4); line-height: 1.7; margin-bottom: 12px; }
.card-bottom { display: flex; align-items: center; justify-content: space-between; margin-bottom: 6px; }
.card-date { font-size: 11px; color: rgba(255,255,255,0.25); }
.tags-row { display: flex; flex-wrap: wrap; gap: 4px; }
.tag-chip { font-size: 10px; padding: 2px 6px; border-radius: 3px; background: rgba(0,255,128,0.06); color: rgba(0,255,128,0.55); }
.card-actions { display: flex; border-top: 1px solid rgba(255,255,255,0.04); margin-top: 14px; padding-top: 12px; gap: 8px; }
.action { flex: 1; padding: 7px; border: 1px solid rgba(255,255,255,0.06); border-radius: 6px; background: none; font-size: 12px; cursor: pointer; transition: all .2s; }
.edit { color: rgba(255,255,255,0.35); }
.edit:hover { border-color: rgba(0,255,128,0.25); color: #00ff80; }
.del { color: rgba(255,255,255,0.2); }
